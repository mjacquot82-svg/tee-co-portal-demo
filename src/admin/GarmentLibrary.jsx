...
function hasExplicitImportedVariantSizes(variants = []) {
  return (Array.isArray(variants) ? variants : []).some(
    (variant) => extractVariantSizeValues(variant).length > 0
  );
}
...
function buildSharedSizeVariantsFromSelections(
  capabilities = [],
  selectedColors = [],
  selectedSizes = [],
  fallbackSizes = [],
  ...
  const normalizedSizes = sortSizesByLookup(
    uniqueList(selectedSizes.length ? selectedSizes : fallbackSizes),
    sizeLookups
  );
...
export default function GarmentLibrary() {
  ...
  const importedSharedSizeValues = useMemo(
    () =>
      sortSizesByLookup(
        uniqueList(importedCapabilitySourceItem?.sizes || form.sizes || EMPTY_LIST),
        sizes
      ),
    [form.sizes, importedCapabilitySourceItem, sizes]
  );
  const importedHasExplicitVariantSizes = useMemo(
    () =>
      hasExplicitImportedVariantSizes(
        importedCapabilitySourceItem?.variants || form.variants || EMPTY_LIST
      ),
    [form.variants, importedCapabilitySourceItem]
  );
  const derivedImportedSizeOptions = useMemo(
    () => {
      if (!importedHasExplicitVariantSizes) {
        return buildImportedSharedSizeOptions(importedSharedSizeValues, sizes);
      }
    ...
    [
      importedCapabilityMatrix,
      importedHasExplicitVariantSizes,
  ...
  async function startCreatingStorefrontProduct(item) {
    ...
    try {
      ...
      console.info("[GarmentLibrary] Create Storefront Product clicked", {
      ...
      console.info("[GarmentLibrary] createStoredProduct resolved", {
        garmentId: item?.id || null,
        garmentTitle: item?.title || "",
        createdProduct,
        currentProductCountAfterCreateCall: products.length,
  ...
  function applyImportedCapabilitySelections(selectedColors, selectedSizes, sourceItem = selectedReusableGarmentEntry?.item) {
    ...
    const sharedSizes = sortSizesByLookup(uniqueList(sourceItem?.sizes || EMPTY_LIST), sizes);
    const usesExplicitVariantSizes = hasExplicitImportedVariantSizes(sourceItem?.variants || EMPTY_LIST);
    ...
    const availableSizeKeys = new Set(
      uniqueList(
        usesExplicitVariantSizes
          ? relevantCapabilities.flatMap((capability) => capability.sizes)
          : sharedSizes
      )
    ...
    setForm((current) => ({
      ...
      variants: usesExplicitVariantSizes
        ? buildAutoGeneratedVariantsFromSelections(
            capabilities,
            normalizedSelectedColors,
            normalizedSelectedSizes,
            sizes
          )
        : buildSharedSizeVariantsFromSelections(
            capabilities,
            normalizedSelectedColors,
            normalizedSelectedSizes,
            sharedSizes,
